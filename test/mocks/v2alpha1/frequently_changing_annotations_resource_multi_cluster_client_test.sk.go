...
var _ = Describe("FrequentlyChangingAnnotationsResourceMultiClusterClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: FrequentlyChangingAnnotationsResourceCrd},
		...
		Context("multi cluster resource client backed by "+test.Description(), func() {
			var (
				client              FrequentlyChangingAnnotationsResourceMultiClusterClient
			...
			It("CRUDs FrequentlyChangingAnnotationsResources "+test.Description(), func() {
				client = NewFrequentlyChangingAnnotationsResourceMultiClusterClient(test)
				FrequentlyChangingAnnotationsResourceMultiClusterClientTest(namespace, client, name1, name2, name3)
			})
			It("errors when no client exists for the given cluster "+test.Description(), func() {
				client = NewFrequentlyChangingAnnotationsResourceMultiClusterClient(test)
				FrequentlyChangingAnnotationsResourceMultiClusterClientCrudErrorsTest(client)
			})
			It("populates an aggregated watch "+test.Description(), func() {
				watchAggregator := wrapper.NewWatchAggregator()
				client = NewFrequentlyChangingAnnotationsResourceMultiClusterClientWithWatchAggregator(watchAggregator, test)
				FrequentlyChangingAnnotationsResourceMultiClusterClientWatchAggregationTest(client, watchAggregator, namespace)
			})
		})
	}
})

func FrequentlyChangingAnnotationsResourceMultiClusterClientTest(namespace string, client FrequentlyChangingAnnotationsResourceMultiClusterClient, name1, name2, name3 string) {
	cfg, err := kubeutils.GetConfig("", "")
	Expect(err).NotTo(HaveOccurred())
	client.ClusterAdded("", cfg)

	name := name1
	input := NewFrequentlyChangingAnnotationsResource(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&FrequentlyChangingAnnotationsResource{}))
	...
	Eventually(func() FrequentlyChangingAnnotationsResourceList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
...
func FrequentlyChangingAnnotationsResourceMultiClusterClientCrudErrorsTest(client FrequentlyChangingAnnotationsResourceMultiClusterClient) {
	_, err := client.Read("foo", "bar", clients.ReadOpts{Cluster: "read"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoFrequentlyChangingAnnotationsResourceClientForClusterError("read").Error()))
	_, err = client.List("foo", clients.ListOpts{Cluster: "list"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoFrequentlyChangingAnnotationsResourceClientForClusterError("list").Error()))
	...
	_, err = client.Write(input, clients.WriteOpts{})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoFrequentlyChangingAnnotationsResourceClientForClusterError("write").Error()))
