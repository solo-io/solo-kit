...
import (
	...
	"github.com/solo-io/solo-kit/test/mocks/v2alpha1/kube/client/clientset/versioned/scheme"
	rest "k8s.io/client-go/rest"
)

type TestingV2alpha1Interface interface {
	RESTClient() rest.Interface
	FrequentlyChangingAnnotationsResourcesGetter
	MockResourcesGetter
}
...
func (c *TestingV2alpha1Client) FrequentlyChangingAnnotationsResources(namespace string) FrequentlyChangingAnnotationsResourceInterface {
	return newFrequentlyChangingAnnotationsResources(c, namespace)
}

func (c *TestingV2alpha1Client) MockResources(namespace string) MockResourceInterface {
	return newMockResources(c, namespace)
}

// NewForConfig creates a new TestingV2alpha1Client for the given config.
...
// NewForConfigAndClient creates a new TestingV2alpha1Client for the given config and http client.
// Note the http client provided takes precedence over the configured transport values.
func NewForConfigAndClient(c *rest.Config, h *http.Client) (*TestingV2alpha1Client, error) {
	config := *c
	if err := setConfigDefaults(&config); err != nil {
		return nil, err
	}
	...
	return &TestingV2alpha1Client{client}, nil
}

// NewForConfigOrDie creates a new TestingV2alpha1Client for the given config and
...
func New(c rest.Interface) *TestingV2alpha1Client {
	return &TestingV2alpha1Client{c}
}
