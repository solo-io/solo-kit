...
var _ = Describe("SimpleMockResourceClient", func() {
	var ctx context.Context
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: SimpleMockResourceCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			var (
				client              SimpleMockResourceClient
				err                 error
			...
			BeforeEach(func() {
				...
				client, err = NewSimpleMockResourceClient(ctx, factory)
			...
			It("CRUDs SimpleMockResources "+test.Description(), func() {
				SimpleMockResourceClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})
...
func SimpleMockResourceClientTest(namespace string, client SimpleMockResourceClient, name1, name2, name3 string) {
	...
	name := name1
	input := NewSimpleMockResource(namespace, name)
	...
	ExpectWithOffset(testOffset, r1).To(BeAssignableToTypeOf(&SimpleMockResource{}))
	...
	Eventually(func() SimpleMockResourceList {
		list, err = client.List(namespace, clients.ListOpts{})
		ExpectWithOffset(testOffset, err).NotTo(HaveOccurred())
		return list
