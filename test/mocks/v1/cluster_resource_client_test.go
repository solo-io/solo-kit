	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs ClusterResources", func() {
				ClusterResourceClientTest(namespace, client)
			})
		})
	}
})

func ClusterResourceClientTest(namespace string, client ClusterResourceClient) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := "foo"
	input := NewClusterResource(namespace, name)
