...
var _ = FDescribe("ClusterResourceClient", func() {
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			AfterEach(func() {
				client.Delete("foo", clients.DeleteOpts{})
				client.Delete("boo", clients.DeleteOpts{})
				client.Delete("goo", clients.DeleteOpts{})
			})
			It("CRUDs ClusterResources", func() {
				ClusterResourceClientTest(client)
			})
		})
	}
})

func ClusterResourceClientTest(client ClusterResourceClient) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
