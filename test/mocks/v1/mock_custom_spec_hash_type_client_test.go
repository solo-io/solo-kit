...
var _ = Describe("MockCustomSpecHashTypeClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewMockCustomSpecHashTypeClient(ctx, factory)
			...
			It("CRUDs MockCustomSpecHashTypes "+test.Description(), func() {
				MockCustomSpecHashTypeClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func MockCustomSpecHashTypeClientTest(namespace string, client MockCustomSpecHashTypeClient, name1, name2, name3 string) {
	testOffset := 1
	...
	name := name1
	input := NewMockCustomSpecHashType(namespace, name)
	...
	ExpectWithOffset(testOffset, r1).To(BeAssignableToTypeOf(&MockCustomSpecHashType{}))
	...
	Eventually(func() MockCustomSpecHashTypeList {
		list, err = client.List(namespace, clients.ListOpts{})
		ExpectWithOffset(testOffset, err).NotTo(HaveOccurred())
		return list
