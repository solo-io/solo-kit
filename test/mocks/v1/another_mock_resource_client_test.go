...
var _ = Describe("AnotherMockResourceClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			var (
				client AnotherMockResourceClient
				err    error
			)
			BeforeEach(func() {
				namespace = helpers.RandString(6)
				factory := test.Setup(namespace)
				client, err = NewAnotherMockResourceClient(factory)
			...
			It("CRUDs AnotherMockResources", func() {
				AnotherMockResourceClientTest(namespace, client)
			})
		})
	}
})

func AnotherMockResourceClientTest(namespace string, client AnotherMockResourceClient) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
