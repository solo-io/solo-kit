...
var _ = Describe("AnotherMockResourceMultiClusterClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: AnotherMockResourceCrd},
		...
		Context("multi cluster resource client backed by "+test.Description(), func() {
			var (
				client              AnotherMockResourceMultiClusterClient
			...
			It("CRUDs AnotherMockResources "+test.Description(), func() {
				client = NewAnotherMockResourceMultiClusterClient(test)
				AnotherMockResourceMultiClusterClientTest(namespace, client, name1, name2, name3)
			})
			It("errors when no client exists for the given cluster "+test.Description(), func() {
				client = NewAnotherMockResourceMultiClusterClient(test)
				AnotherMockResourceMultiClusterClientCrudErrorsTest(client)
			})
			It("populates an aggregated watch "+test.Description(), func() {
				watchAggregator := wrapper.NewWatchAggregator()
				client = NewAnotherMockResourceMultiClusterClientWithWatchAggregator(watchAggregator, test)
				AnotherMockResourceMultiClusterClientWatchAggregationTest(client, watchAggregator, namespace)
			})
		})
	}
})

func AnotherMockResourceMultiClusterClientTest(namespace string, client AnotherMockResourceMultiClusterClient, name1, name2, name3 string) {
...
func AnotherMockResourceMultiClusterClientCrudErrorsTest(client AnotherMockResourceMultiClusterClient) {
	...
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoAnotherMockResourceClientForClusterError("read").Error()))
	_, err = client.List("foo", clients.ListOpts{Cluster: "list"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoAnotherMockResourceClientForClusterError("list").Error()))
	err = client.Delete("foo", "bar", clients.DeleteOpts{Cluster: "delete"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoAnotherMockResourceClientForClusterError("delete").Error()))
	...
	_, err = client.Write(input, clients.WriteOpts{})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoAnotherMockResourceClientForClusterError("write").Error()))
	_, _, err = client.Watch("foo", clients.WatchOpts{Cluster: "watch"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoAnotherMockResourceClientForClusterError("watch").Error()))
}
func AnotherMockResourceMultiClusterClientWatchAggregationTest(client AnotherMockResourceMultiClusterClient, aggregator wrapper.WatchAggregator, namespace string) {
