	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewDeploymentClient(ctx, factory)
			...
			It("CRUDs Deployments "+test.Description(), func() {
				DeploymentClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})
...
func DeploymentClientTest(namespace string, client DeploymentClient, name1, name2, name3 string) {
	...
	ExpectWithOffset(testOffset, r1).To(BeAssignableToTypeOf(&Deployment{}))
