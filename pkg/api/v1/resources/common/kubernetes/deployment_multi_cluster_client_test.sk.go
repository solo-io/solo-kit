...
var _ = Describe("DeploymentMultiClusterClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		...
		Context("multi cluster resource client backed by "+test.Description(), func() {
			var (
				client              DeploymentMultiClusterClient
			...
			It("CRUDs Deployments "+test.Description(), func() {
				client = NewDeploymentMultiClusterClient(test)
				DeploymentMultiClusterClientTest(namespace, client, name1, name2, name3)
			})
			It("errors when no client exists for the given cluster "+test.Description(), func() {
				client = NewDeploymentMultiClusterClient(test)
				DeploymentMultiClusterClientCrudErrorsTest(client)
			})
			It("populates an aggregated watch "+test.Description(), func() {
				watchAggregator := wrapper.NewWatchAggregator()
				client = NewDeploymentMultiClusterClientWithWatchAggregator(watchAggregator, test)
				DeploymentMultiClusterClientWatchAggregationTest(client, watchAggregator, namespace)
			})
		})
	}
})

func DeploymentMultiClusterClientTest(namespace string, client DeploymentMultiClusterClient, name1, name2, name3 string) {
	cfg, err := kubeutils.GetConfig("", "")
	Expect(err).NotTo(HaveOccurred())
	client.ClusterAdded("", cfg)

	name := name1
	input := NewDeployment(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&Deployment{}))
	...
	Eventually(func() DeploymentList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
...
func DeploymentMultiClusterClientCrudErrorsTest(client DeploymentMultiClusterClient) {
	...
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoDeploymentClientForClusterError("read").Error()))
	_, err = client.List("foo", clients.ListOpts{Cluster: "list"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoDeploymentClientForClusterError("list").Error()))
	err = client.Delete("foo", "bar", clients.DeleteOpts{Cluster: "delete"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoDeploymentClientForClusterError("delete").Error()))
	...
	_, err = client.Write(input, clients.WriteOpts{})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoDeploymentClientForClusterError("write").Error()))
	_, _, err = client.Watch("foo", clients.WatchOpts{Cluster: "watch"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoDeploymentClientForClusterError("watch").Error()))
}
func DeploymentMultiClusterClientWatchAggregationTest(client DeploymentMultiClusterClient, aggregator wrapper.WatchAggregator, namespace string) {
