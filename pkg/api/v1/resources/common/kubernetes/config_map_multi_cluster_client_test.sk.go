...
var _ = Describe("ConfigMapMultiClusterClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		...
		Context("multi cluster resource client backed by "+test.Description(), func() {
			var (
				client              ConfigMapMultiClusterClient
			...
			It("CRUDs ConfigMaps "+test.Description(), func() {
				client = NewConfigMapMultiClusterClient(test)
				ConfigMapMultiClusterClientTest(namespace, client, name1, name2, name3)
			})
			It("errors when no client exists for the given cluster "+test.Description(), func() {
				client = NewConfigMapMultiClusterClient(test)
				ConfigMapMultiClusterClientCrudErrorsTest(client)
			})
			It("populates an aggregated watch "+test.Description(), func() {
				watchAggregator := wrapper.NewWatchAggregator()
				client = NewConfigMapMultiClusterClientWithWatchAggregator(watchAggregator, test)
				ConfigMapMultiClusterClientWatchAggregationTest(client, watchAggregator, namespace)
			})
		})
	}
})

func ConfigMapMultiClusterClientTest(namespace string, client ConfigMapMultiClusterClient, name1, name2, name3 string) {
	cfg, err := kubeutils.GetConfig("", "")
	Expect(err).NotTo(HaveOccurred())
	client.ClusterAdded("", cfg)

	name := name1
	input := NewConfigMap(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&ConfigMap{}))
...
func ConfigMapMultiClusterClientCrudErrorsTest(client ConfigMapMultiClusterClient) {
	...
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoConfigMapClientForClusterError("read").Error()))
	_, err = client.List("foo", clients.ListOpts{Cluster: "list"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoConfigMapClientForClusterError("list").Error()))
	err = client.Delete("foo", "bar", clients.DeleteOpts{Cluster: "delete"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoConfigMapClientForClusterError("delete").Error()))
	...
	_, err = client.Write(input, clients.WriteOpts{})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoConfigMapClientForClusterError("write").Error()))
	_, _, err = client.Watch("foo", clients.WatchOpts{Cluster: "watch"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoConfigMapClientForClusterError("watch").Error()))
}
func ConfigMapMultiClusterClientWatchAggregationTest(client ConfigMapMultiClusterClient, aggregator wrapper.WatchAggregator, namespace string) {
