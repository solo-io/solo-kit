...
var _ = Describe("ServiceMultiClusterClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		...
		Context("multi cluster resource client backed by "+test.Description(), func() {
			var (
				client              ServiceMultiClusterClient
			...
			It("CRUDs Services "+test.Description(), func() {
				client = NewServiceMultiClusterClient(test)
				ServiceMultiClusterClientTest(namespace, client, name1, name2, name3)
			})
			It("errors when no client exists for the given cluster "+test.Description(), func() {
				client = NewServiceMultiClusterClient(test)
				ServiceMultiClusterClientCrudErrorsTest(client)
			})
			It("populates an aggregated watch "+test.Description(), func() {
				watchAggregator := wrapper.NewWatchAggregator()
				client = NewServiceMultiClusterClientWithWatchAggregator(watchAggregator, test)
				ServiceMultiClusterClientWatchAggregationTest(client, watchAggregator, namespace)
			})
		})
	}
})

func ServiceMultiClusterClientTest(namespace string, client ServiceMultiClusterClient, name1, name2, name3 string) {
	...
	Expect(r1).To(BeAssignableToTypeOf(&Service{}))
...
func ServiceMultiClusterClientCrudErrorsTest(client ServiceMultiClusterClient) {
	_, err := client.Read("foo", "bar", clients.ReadOpts{Cluster: "read"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoServiceClientForClusterError("read").Error()))
	_, err = client.List("foo", clients.ListOpts{Cluster: "list"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoServiceClientForClusterError("list").Error()))
	err = client.Delete("foo", "bar", clients.DeleteOpts{Cluster: "delete"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoServiceClientForClusterError("delete").Error()))
	...
	_, err = client.Write(input, clients.WriteOpts{})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoServiceClientForClusterError("write").Error()))
	_, _, err = client.Watch("foo", clients.WatchOpts{Cluster: "watch"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoServiceClientForClusterError("watch").Error()))
}
func ServiceMultiClusterClientWatchAggregationTest(client ServiceMultiClusterClient, aggregator wrapper.WatchAggregator, namespace string) {
