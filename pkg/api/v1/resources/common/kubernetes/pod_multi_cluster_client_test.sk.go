...
var _ = Describe("PodMultiClusterClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		...
		Context("multi cluster resource client backed by "+test.Description(), func() {
			...
			It("CRUDs Pods "+test.Description(), func() {
				client = NewPodMultiClusterClient(test)
				PodMultiClusterClientTest(namespace, client, name1, name2, name3)
			})
			It("errors when no client exists for the given cluster "+test.Description(), func() {
				client = NewPodMultiClusterClient(test)
				PodMultiClusterClientCrudErrorsTest(client)
			})
			It("populates an aggregated watch "+test.Description(), func() {
				watchAggregator := wrapper.NewWatchAggregator()
				client = NewPodMultiClusterClientWithWatchAggregator(watchAggregator, test)
				PodMultiClusterClientWatchAggregationTest(client, watchAggregator, namespace)
			})
		})
	}
})

func PodMultiClusterClientTest(namespace string, client PodMultiClusterClient, name1, name2, name3 string) {
	cfg, err := kubeutils.GetConfig("", "")
	Expect(err).NotTo(HaveOccurred())
	client.ClusterAdded("", cfg)

	name := name1
	input := NewPod(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&Pod{}))
...
func PodMultiClusterClientCrudErrorsTest(client PodMultiClusterClient) {
	_, err := client.Read("foo", "bar", clients.ReadOpts{Cluster: "read"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoPodClientForClusterError("read").Error()))
	_, err = client.List("foo", clients.ListOpts{Cluster: "list"})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoPodClientForClusterError("list").Error()))
	...
	_, err = client.Write(input, clients.WriteOpts{})
	Expect(err).To(HaveOccurred())
	Expect(err.Error()).To(Equal(NoPodClientForClusterError("write").Error()))
