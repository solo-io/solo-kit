...
func (rc *ResourceClient) RenewToken(client *vault.Client, awsAuth *auth.AWSAuth, watcherIncrement int) {
	contextutils.LoggerFrom(context.Background()).Warnf("Starting renewToken goroutine")
	for {
		vaultLoginResp, err := rc.vault.Auth().Login(context.Background(), awsAuth)
		if err != nil {
			log.Fatalf("unable to authenticate to Vault: %v", err)
		}
		tokenErr := rc.manageTokenLifecycle(vaultLoginResp, watcherIncrement)
		if tokenErr != nil {
...
func (rc *ResourceClient) manageTokenLifecycle(token *vault.Secret, watcherIncrement int) error {
	...
	watcher, err := rc.vault.NewLifetimeWatcher(lifetimeWatcherInput)
	if err != nil {
