...
func GenerateFiles(project *model.CliProject, skipOutOfPackageFiles bool) (model.Files, error) {
	...
	for _, res := range project.Resources {
		...
		//	continue
		//}
		fs, err := generateFilesForResource(res, project.Path)
		if err != nil {
			return nil, err
		}
...
func generateFilesForResource(resource *model.Resource, cliDir string) (model.Files, error) {
	...
	for _, commandType := range []string{"root", "get"} {
		...
		err := commandFileInfo.AddImport(filepath.Join(cliDir, "options"))
		if err != nil {
			return files, err
		}
...
func generateFilesForProject(project *model.CliProject) (model.Files, error) {
	var v model.Files
	...
	err := mainFileInfo.AddImport(filepath.Join(project.Path, "options"), filepath.Join(project.Path, "cmd"))
	if err != nil {
		return nil, err
	}
	...
	cmdImports = append(cmdImports, filepath.Join(project.Path, "options"))
	for _, v := range project.Resources {
		cmdImports = append(cmdImports, filepath.Join(project.Path, "cmd")+v.Name)
	}
...
func generateClientHelpers(project *model.CliProject) (model.File, error) {
	var file model.File
	var err error
...
func generatePrinters(project *model.CliProject) (model.Files, error) {
	var files model.Files
	var err error
	for _, res := range project.Resources {
...
func generateCliFile(cfg *model.CliFile, tmpl *template.Template) (model.File, error) {
	var file model.File
	buf := &bytes.Buffer{}
...
func generateCliResourceFile(cfg *model.CliResourceFile, tmpl *template.Template) (model.File, error) {
	var file model.File
	buf := &bytes.Buffer{}
	if err := tmpl.Execute(buf, cfg); err != nil {
		return file, err
