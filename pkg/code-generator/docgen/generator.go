...
func (d *DocsGen) hugoFileHeader(filename string) string {
	...
	// For project files:
	//	- The title should not include the project.sk.md suffix
	//	- The title will remain as [group].[kind].solo.io
	// This means that custom_project.solo.sk.md will have a title of custom_project.solo
	projectExtension := ".project" + d.protoSuffix()
	if strings.HasSuffix(name, projectExtension) {
		name = strings.TrimSuffix(name, projectExtension)
		title = name
	}
